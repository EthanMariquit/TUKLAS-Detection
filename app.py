...
class PDFReport(FPDF):
    def header(self):
        ...
        # 4. Horizontal Line
        self.set_y(40) # Safety buffer to prevent overlap
...
def create_pdf(image_paths, diagnosis, confidence, info):
    """
    image_paths: List of 4 file paths ['orig', 'annotated', 'contrast', 'edge']
    """
    pdf = PDFReport()
    ...
    # --- SECTION 2: MULTI-VIEW ANALYSIS (The Cool Grid) ---
    pdf.set_font("Arial", "B", 10)
    pdf.cell(0, 6, "MULTI-SPECTRAL ANALYSIS", 0, 1, 'L')
    
    # Draw Container Box
    y_start = pdf.get_y()
    pdf.rect(10, y_start, 190, 45) # Height 45 to fit images + labels
    
    # Grid Logic: 4 images in one row
    # X positions: 12, 59, 106, 153 (Approx spread across 190mm width)
    ...
    pdf.set_xy(10, y_start + 45) # Move cursor past the box
    pdf.ln(3)
        ...
        if st.button("🔍 Generate Report"):
            if model is None:
                ...
                with st.spinner("Analyzing Specimen..."):
                    ...
                    # 3. Save Contrast Enhanced (Clinical View)
                    enhancer = ImageEnhance.Contrast(img)
                    img_contrast = enhancer.enhance(1.5) # Increase contrast
                    img_contrast.save("temp_contrast.jpg")
                    
                    # 4. Save Structural/Edge View (Texture View)
                    img_gray = ImageOps.grayscale(img)
                    img_edge = img_gray.filter(ImageFilter.FIND_EDGES)
                    img_edge = ImageOps.invert(img_edge) # Invert to look like pencil sketch
